            ...
            <div class="staff-card" onclick="openProfile('${u.username}')">
                <div style="display:flex; align-items:center; gap:10px;">
                    <div style="width:35px; height:35px; background:#f0f0f0; border-radius:50%; display:flex; align-items:center; justify-content:center; overflow:hidden;">
                        ${u.img ? `<img src="${u.img}" style="width:100%; height:100%; object-fit:cover;">` : `<span style="font-weight:800; color:#e60000; font-size:14px;">${u.fullname[0]}</span>`}
                    </div>
                    <div>
                        <b style="font-size:14px;">${u.fullname} ${u.isVerified ? blueTickSVG : ''}</b>
                        <div style="font-size:11px; color:#888;">@${u.username}</div>
                    </div>
                </div>
                <span style="color:#e60000; font-weight:bold;">→</span>
            </div>`).join("");
};
...
window.openProfile = async function(username) {
    ...
    if (u.img) { 
        ...
        initialDiv.innerText = u.fullname[0]; 
...
window.triggerPhotoUpload = function() {
    ...
    input.onchange = async (e) => {
        ...
        // Şəkil ölçüsünü yoxla (Firebase üçün çox böyük olmasın)
        if (file.size > 1024 * 1024) { alert("Şəkil çox böyükdür (max 1MB)"); return; }
...
window.createNewPost = async function() {
    ...
    if (!text) return;
    const newPost = { author: currentLogin, text: text, date: new Date().toLocaleString() };
    await fetch(`${POSTS_URL}.json`, { method: 'POST', body: JSON.stringify(newPost) });
...
window.changeUserPassword = async function() {
    ...
    if (newPass && newPass.length >= 4) {
        await fetch(`${DB_URL}/${currentLogin}.json`, { method: 'PATCH', body: JSON.stringify({ password: newPass }) });
